...
CONNECT TO OLEDB  AS CDW1 ( PROVIDER=SQLNCLI11  DATASOURCE=&data_source.
   ...
   CONNECTION=GLOBAL ) ; 
create table sepsis.LabChemFinalYEAR as select * FROM CONNECTION TO CDW1 
...
FROM [INSERT STUDY NAME].[dflt].[LabChemFinalYEAR] as t1) as t1;
...
create table sepsis.OrderedCulturesYYYY as select * FROM CONNECTION TO CDW1 
(SELECT t1.* FROM [INSERT STUDY NAME].[dflt].[OrderedCulturesYYYY] as t1) as t1;
DISCONNECT FROM CDW1 ;
quit; 
...
CONNECT TO OLEDB  AS CDW1 ( PROVIDER=SQLNCLI11  DATASOURCE=&data_source.
   ...
   CONNECTION=GLOBAL ); 
	...
	from bloodcult a
	left join [INSERT STUDY NAME].[src].[SPatient_SPatient] b on a.PatientSID=b.PatientSID;
);
DISCONNECT FROM CDW1;
QUIT;
